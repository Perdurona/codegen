...
def create_pr_agent(
    ...
    **kwargs
) -> Union[Dict[str, Any], PRCoderObservation]:
    ...
    Returns:
        Dictionary with PR information or PRCoderObservation containing the result
        of the PR creation depending on which approach is used
    """
    try:
        ...
        # Create a unique branch for the changes if not provided
        if not branch_name:
        ...
        if use_reflection:
            ...
            # Use the simplified approach without reflection
            return agent.generate_pr(
                user_request=user_request,
                branch_name=branch_name,
                base_branch=base_branch,
            )
...
if __name__ == "__main__":
    ...
    if isinstance(result, PRCoderObservation):
        print(result.render_as_string())
    else:
        print(f"PR created with branch: {result.get('branch_name')}")
